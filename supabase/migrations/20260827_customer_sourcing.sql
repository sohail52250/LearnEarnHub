-- LearnEarnHub: Customer sourcing workflow
-- Non-government marketplace workflow:
-- customer need -> discovery -> quotation -> comparison -> order -> delivery -> confirmation -> payment
-- Non-destructive: creates new tables/policies/indexes only.
...
create table if not exists public.sourcing_quotations (
    ...
    supplier_contact_whatsapp text,
...
create table if not exists public.sourcing_orders (
    ...
    customer_contact_whatsapp text,
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
);
...
create table if not exists public.sourcing_deliveries (
    id uuid primary key default gen_random_uuid(),
    order_id uuid unique not null references public.sourcing_orders(id) on delete cascade,
...
-- Requests are private to their customer. Anonymous users may not read or write them.
drop policy if exists sourcing_requests_owner_select on public.sourcing_requests;
create policy sourcing_requests_owner_select on public.sourcing_requests
for select to authenticated
using (customer_id = auth.uid());

drop policy if exists sourcing_requests_owner_insert on public.sourcing_requests;
create policy sourcing_requests_owner_insert on public.sourcing_requests
for insert to authenticated
with check (customer_id = auth.uid());

drop policy if exists sourcing_requests_owner_update on public.sourcing_requests;
create policy sourcing_requests_owner_update on public.sourcing_requests
for update to authenticated
using (customer_id = auth.uid())
with check (customer_id = auth.uid());

-- Supplier quotations are visible to the request owner; suppliers can manage their own submitted quotes.
drop policy if exists sourcing_quotes_request_owner_select on public.sourcing_quotations;
create policy sourcing_quotes_request_owner_select on public.sourcing_quotations
for select to authenticated
using (
    exists (select 1 from public.sourcing_requests r where r.id = request_id and r.customer_id = auth.uid())
    or exists (select 1 from public.businesses b where b.id = supplier_business_id and b.owner_id = auth.uid())
);

drop policy if exists sourcing_quotes_supplier_insert on public.sourcing_quotations;
create policy sourcing_quotes_supplier_insert on public.sourcing_quotations
for insert to authenticated
with check (
    exists (
        select 1 from public.businesses b
        where b.id = supplier_business_id
...
create policy sourcing_quotes_supplier_update on public.sourcing_quotations
for update to authenticated
using (exists (select 1 from public.businesses b where b.id = supplier_business_id and b.owner_id = auth.uid()))
with check (exists (select 1 from public.businesses b where b.id = supplier_business_id and b.owner_id = auth.uid()));

-- Orders/deliveries are private. Service/API layer enforces customer/supplier ownership.
drop policy if exists sourcing_orders_participant_select on public.sourcing_orders;
create policy sourcing_orders_participant_select on public.sourcing_orders
for select to authenticated
using (
...
create policy sourcing_deliveries_participant_select on public.sourcing_deliveries
for select to authenticated
using (
    exists (
        select 1 from public.sourcing_orders o
        where o.id = order_id
          and (o.customer_id = auth.uid() or exists (select 1 from public.businesses b where b.id = o.supplier_business_id and b.owner_id = auth.uid()))
    )
);

-- selected_quote_id is intentionally not an FK: quotations are created independently and can be compared before selection.
